...
namespace Es.Riam.Gnoss.Web.Controles.Administracion
{
    public class ControladorPestanyas : ControladorBase
    {
			...
			if (filasBusqueda != null)
			{
				...
				switch ((TipoPestanyaMenu)filaPestanya.TipoPestanya)
				{
					...
					case TipoPestanyaMenu.PersonasYOrganizaciones:
						campoFiltro = $"rdf:type={FacetadoAD.BUSQUEDA_PERSONASYORG}|rdf:type={FacetadoAD.BUSQUEDA_CLASE}|rdf:type={FacetadoAD.BUSQUEDA_ORGANIZACION}|rdf:type={FacetadoAD.BUSQUEDA_PERSONA}|rdf:type={FacetadoAD.BUSQUEDA_GRUPO}";
						break;
				...
				if (filasFiltroOrden.Any())
				{
					foreach (ProyectoPestanyaFiltroOrdenRecursos filaFiltroOrden in filasFiltroOrden.OrderBy(filtroOrden => filtroOrden.Orden))
					{
						...
						filtroOrden.Consulta = filaFiltroOrden.Consulta;
						filtroOrden.OrderBy = filaFiltroOrden.OrderBy;
				...
				if (!pestanya.Deleted && !listaPestanyasNuevas.Contains(pestanya.Key))
				{
					...
					if (pestanya.Modified)
					{
						...
						if (pestanya.OpcionesBusqueda != null)
						{
							...
							if (pestanya.OpcionesBusqueda.CampoFiltro.EndsWith("|"))
							{
								pestanya.OpcionesBusqueda.CampoFiltro = pestanya.OpcionesBusqueda.CampoFiltro.Substring(0, pestanya.OpcionesBusqueda.CampoFiltro.Length - 1);
							}

							if(pestanya.OpcionesBusqueda.RelacionMandatory == null)
							{
								pestanya.OpcionesBusqueda.RelacionMandatory = string.Empty;
							}
		...
		private void GuardarDatosFiltroOrdenNueva(ProyectoPestanyaFiltroOrdenRecursos pFilaFiltroOrden, TabModel.SearchTabModel.FiltroOrden pFiltroOrden)
		{
			...
			pFilaFiltroOrden.Consulta = pFiltroOrden.Consulta;
			pFilaFiltroOrden.OrderBy = pFiltroOrden.OrderBy;
		}
				...
				foreach (TabModel.ExportacionSearchTabModel exportacion in pPestanya.ListaExportaciones.ToList())
				{
					...
					else if (!exportacion.Deleted)
					{
						...
						if (exportacion.ListaPropiedades != null)
						{
							...
							List<string> propiedadesYaAniadidas = new List<string>();
							...
							{
								if (!propiedadesYaAniadidas.Contains(propiedad.Nombre))
								{
									...
									if (filaPropiedadExportacion == null)
									{
										...
										propiedadesYaAniadidas.Add(propiedad.Nombre);
									}
		...
		private void ReordenarFilasPropiedadesExportacion(ProyectoPestanyaBusquedaExportacion filaExportacion)
		{
			...
			if (filas.Any())
			{
				...
				{
					if (fila.Orden != orden && !propiedadesYaAniadidas.Contains(fila.Propiedad))
					{
					...
					propiedadesYaAniadidas.Add(fila.Propiedad);
					orden++;
				}
				mEntityContext.SaveChanges();
			}
