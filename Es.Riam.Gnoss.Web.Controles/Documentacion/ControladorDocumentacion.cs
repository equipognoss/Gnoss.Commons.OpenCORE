...
namespace Es.Riam.Gnoss.Web.Controles.Documentacion
{
    ...
    public class ControladorDocumentacion : ControladorBase
    {
        ...
        /// <summary>
        /// Aplica la mejora introduciondo los datos del rdf en Virtuoso (ontologia y busqueda)
        /// </summary>
        /// <param name="pDocumentoOriginal">Documentor original de la mejora</param>
        /// <param name="pDocumentoMejora">Documento de la mejora aceptada</param>
        /// <param name="pAvailableServices">Lista de servicios disponibles</param>
        public void AplicarMejoraDocumentoRDF(Documento pDocumentoOriginal, Documento pDocumentoMejora, IAvailableServices pAvailableServices)
        {
            string rdfDocumentoMejora = ObtenerTextoRDFDeBDRdfHistorico(pDocumentoMejora.Clave, GestionOWL.NAMESPACE_ONTO_GNOSS);
            ActualizarDatosVirtuoso(pDocumentoOriginal, pDocumentoMejora, rdfDocumentoMejora, pAvailableServices);
        }

        /// <summary>
        /// Actualiza los datos del recurso en Virtoso de forma que se quedan los de la nueva versión
        /// </summary>
        /// <param name="pDocumentoOriginal">La versión actual deldocumento</param>
        /// <param name="pDocumentoNuevo">La versión que se desea aplicar/restaurar</param>
        /// <param name="rdfDocumentoNuevo">El rdf a guardar en Virtuoso</param>
        private void ActualizarDatosVirtuoso(Documento pDocumentoOriginal, Documento pDocumentoNuevo, string rdfDocumentoNuevo, IAvailableServices pAvailableServices)
        {
            ...
            List<ElementoOntologia> instanciasPrincipales = gestorOWL.LeerFicheroRDF(ontologia, rdfDocumentoNuevo, true);
        ...
        /// </summary>
        /// <param name="pOntologiaID">ID de la ontología</param>
        ...
        public GestorDocumental CargaIncialDeBaseRecursos(GestorDocumental pGestorDocumental, Identidad pIdentidadOrganizacion, bool pBaseRecursosComunidad, Guid pProyectoID, Guid pUsuarioID, Guid pOrganizacionID)
        {
            ...
            //Lo devolvemos tal y como estaba si no había que recargar o uno nuevo con los datos cargados:
            ...
            foreach (AD.EntityModel.Models.Documentacion.DocumentoWebVinBaseRecursos filaDocVinWeb in pDocumento.FilaDocumento.DocumentoWebVinBaseRecursos)
            {
                if (pDocumento.FilaDocumento.Eliminado || !pDocumento.FilaDocumento.UltimaVersion || filaDocVinWeb.Eliminado) //Si esta eliminado el recursos o descompartido en la BR
                {
                    ...
                    if (proyectoActualizarID != Guid.Empty)
                    {
                        ...
                        // En los grafos está guardado con el ID original
        ...
        /// <summary>
        /// Notifica la compatición de uno o más documentos en una comunidad
        /// </summary>
        ...
        public void NotificarAgregarRecursosEnComunidad(List<Guid> pDocumentos, Guid pComunidadID, PrioridadBase pPrioridadBase, long pEstadoCargaID, IAvailableServices pAvailableServices)
        {
            ...
            foreach (Guid docID in pDocumentos)
            {
                ...
                // Bug 9325 - Buscar en el código este bug puesto que si esto se quita de aquí, en otros sitios no se va a agregar al base ninguna fila (eliminiaciones)
                if (mEliminadoAsinc)
                {
        ...
        /// <param name="pTipoElemento">Tipo de elemento que se está tratando</param>
        /// <param name="pPrioridad">Prioridad</param>
        /// <param name="pInfoExtra">Información extra</param>
        ...
        /// <param name="pBaseURLContent">Url content</param>
        /// <param name="pTieneImagen">Verdad si el documento tiene una imagen generada</param>
        /// <param name="pComplemento">Texto que se añadirá a cada nombre de la imagen para variar esta. Ej: Tamaño del documento si se especifica, null o Empty si es por defecto</param>
        /// <param name="pVersionImagen">Version de la imagen (null si no es tipo foto o enlace)</param>        
        /// <param name="pTamanioImagen">Tamaño de la imagen, por defecto</param>
        /// <returns>Ruta de la imagen de un documento</returns>
        ...
        /// <summary>
        /// Obtiene la URL de la imagen principal si la tiene de un recurso semántico.
        /// </summary>
        /// <param name="pDocumento">Documento</param>
        /// <param name="pPage">Página</param>
        /// <returns></returns>
        ...
        /// <returns>Función javascript para registrar en Google Analytic una acción</returns>
        ...
        /// <param name="pSource">Source of the thesaurus to romve</param>
        /// <param name="pProyectoID">Identifier of the project</param>
        /// <param name="pFacetadoCN">FacetadoCN initialized</param>

        public void EliminarTesauroOntologiaBusqueda(string pOntologia, string pSource, Guid pProyectoID, FacetadoCN pFacetadoCN)
        {
        ...
        /// <param name="pEditandoEntSec">Indica si se está editando la entidad secundaria o de lo contrario es nueva</param>
        public void GuardarRDFEntidadSecundaria(List<ElementoOntologia> pEntidades, string pUrlIntragnoss, string pUrlOntologia, Guid pOrgProyID, Guid pProyectoID, Documento pDocOnto, bool pEditandoEntSec)
        {
            ...
            {
                throw new ExcepcionGeneral($"Ya existe una entidad secundaria con ID '{pEntidades[0].Uri}' en la ontología '{pUrlOntologia}'.");
            }
            ...
            foreach (TripleWrapper triple in tripletasInsertadasDSList)
            {
                ...
                {
                    //Si hay un problema al generar un triple no detenemos la ejecución y generamos el resto
                }
            }
        ...
        /// <summary>
        /// Agrega la tripleta que indica que una ontología secundaria tiene una entidad.
        /// </summary>
